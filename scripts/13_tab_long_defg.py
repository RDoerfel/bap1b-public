...
for analysis in ["de", "fg"]:
    ...
    df["target"] = df["target"].str.replace("hippocampus_icv", "Hippocampus/ICV")
    ...
    # Aggregate to ensure unique index combinations before pivot
    df = df.groupby(["Model", "diagnosis", "target"], as_index=False)[["ar (95%)", "p-value"]].first()
