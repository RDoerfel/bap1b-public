...
data_subset <- data_subset %>%
    ...
    dplyr::filter(dplyr::first(session_id) == "M000" & dplyr::first(diagnosis) == "MCI") %>%
    dplyr::ungroup()

# drop all CN
data_subset <- data_subset %>%
    dplyr::filter(diagnosis != "CN")
...
# save to excel
store_path <- here::here("data", "data_long_c_mci.xlsx")
...
# write to excel
store_results_path <- here::here("results", "long_c_results_mci.xlsx")
writexl::write_xlsx(total_result, path = store_results_path)

# write total_preds to excel
store_preds_path <- here::here("results", "long_c_preds_mci.xlsx")
...
store_roc_path <- here::here("results", "long_c_roc_data_mci.xlsx")
