...
# %% ### load data
df_results = pd.read_excel(results_dir / "long_c_results_mci.xlsx")
df_preds = pd.read_excel(results_dir / "long_c_preds_mci.xlsx")

df_data = pd.read_excel(data_dir / "data_long_c_mci.xlsx")
...
for i, model in enumerate(models):
    ...
    # plot expit
    ax.plot(x, y, color=color_dict_lines["MCI"])

    # plot confidence intervall
    ax.fill_between(x, y_cinf_lower, y_cinf_upper, color=color_dict_shade["MCI"], alpha=0.9)
...
# %% ROC curve
roc_data = pd.read_excel(results_dir / "long_c_roc_data_mci.xlsx")
